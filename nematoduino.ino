...
/*
 * Harsh touch on tail:
 * https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3098610/
 * 
 * Laser ablation of PDE also led to a strong defect in posterior response (Figure 3c), 
 * indicating that PDE plays an important role in this area. 
 * The observation that the loss of PVD or PDE reduced rather than abolished posterior response 
 * prompted us to reason that these two neurons may play a redundant role in posterior harsh touch sensation. 
 * Indeed, animals lacking both PVD and PDE failed to move forward upon posterior harsh touch (Figure 3c).
 * 
 * Although additional neurons are involved, 
 * these data identify PHA/PHB as the primary sensory neurons in mediating anus harsh touch sensation.
 * 
 */
  if (SensorTailRightDistance()< 25) {
  //  PingNeuron(N_PVCR);
    PingNeuron(N_PVDR);
    PingNeuron(N_PDER);
  }
